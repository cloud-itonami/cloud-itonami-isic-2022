(ns paintmfg.sim
  ...
  dispersion or tinting/filling line directly (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product type, a
  production-batch patch with an implausible viscosity reading, a
  production-batch patch with an implausible fineness-of-grind
  reading, and a production-batch patch whose declared VOC content
  exceeds its product type's own regulatory ceiling.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on disperser-001 (verified, registered, high-speed-disperser -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on disperser-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on mill-002 (UNVERIFIED/unregistered bead mill -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on disperser-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible viscosity reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:viscosity-cp -5.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible fineness-of-grind reading -> HARD hold ==")
    (println (exec-op actor "t14"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:fineness-of-grind-hegman 12.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with a VOC content exceeding the interior-flat-paint regulatory ceiling -> HARD hold ==")
