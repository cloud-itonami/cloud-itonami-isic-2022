(ns paintmfg.advisor
  "PaintAdvisor -- the *contained intelligence node* for the paints,
  varnishes and similar coatings, printing ink and mastics plant-
  operations coordination actor.

  It normalizes production-batch patches (product-type/weight/
  viscosity/VOC-content/fineness-of-grind pigment-dispersion-quality),
  drafts a mixing/dispersion-equipment (high-speed disperser, bead/
  ball mill) or tinting/filling-line maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag, and
  drafts an outbound shipment coordination proposal against a
  ...
  a committed record and NEVER a real mixing/dispersion-line actuation
  or freight dispatch. Every output is censored downstream by
  `paintmfg.governor` before anything touches the SSoT -- see README
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct mixing/
                                 ; dispersion-line-control effect
  ...
  contract every caller of this actor agrees to) -- `paintmfg.governor`
...
(defn- log-production-batch
  ...
  weight, viscosity, VOC-content, fineness-of-grind, or verification
...
(defn- schedule-maintenance
  "Draft a mixing/dispersion-equipment (high-speed disperser, bead/
  ball mill) or tinting/filling-line maintenance-window scheduling
  ...
  `paintmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a chemical-hazard (solvent VOC exposure)/flammability/
  equipment-safety concern. ALWAYS `:stake :coordination/safety-
  ...
  on an administrative technicality). See `paintmfg.phase`: no phase
  ever adds this op to a phase's `:auto` set; `paintmfg.governor` also
...
(defn- coordinate-shipment
  "Draft an outbound paint/varnish/coating/printing-ink/mastic
  ...
  NOT invent one, and `paintmfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは塗料・ワニス・類似コーティング・印刷インキ・マスチック"
       "工場プラント運用コーディネーターの助言者です。"
       ...
       "分散機・ボールミル/ビーズミル・調色/充填ライン等の設備の直接操作(actuate)を"
       "絶対に提案してはいけません(この actor は提案のみを行い、実行は一切"
       "行いません)。出荷量を偽って報告してはいけません。VOC含有量規制上限を"
       "超過するバッチをコンプライアンス適合と偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `paintmfg.governor`
