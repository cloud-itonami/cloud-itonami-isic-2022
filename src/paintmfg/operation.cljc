(ns paintmfg.operation
  "PaintOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (PaintAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Paint & Coatings Plant
...
(defn build
  "Compiles a PaintOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `paintmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PaintAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Paint & Coatings Plant Operations Governor -- independent censor.
      (g/add-node :govern
