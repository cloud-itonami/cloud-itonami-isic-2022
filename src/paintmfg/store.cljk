(ns paintmfg.store
  "SSoT for the paints, varnishes and similar coatings, printing ink
  and mastics plant-operations coordination actor, behind a `Store`
  ...
  (`cloud-itonami-isic-2023`'s own `soapmfg.store`), this build ships
  ...
  Decision 1, this vertical is self-contained (no external paint/
  coatings/ink/mastic-manufacturing capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/weight/viscosity/VOC-content/
                             fineness-of-grind record. `:verified?`
    ...
    - `equipment`         -- a mixing/dispersion (high-speed disperser,
                             bead/ball mill), tinting, or filling-line
    ...
    - `maintenance`       -- a scheduled mixing/dispersion/tinting/
                             filling-line maintenance-window DRAFT
    ...
    - `shipments`         -- a proposed outbound paint/varnish/
                             coating/printing-ink/mastic shipment DRAFT
                             (`paintmfg.registry`'s `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (paintmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (paintmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :interior-flat-paint :output-form :liquid
                :material "Acrylic-Latex Interior Flat Wall Paint"
                :weight-kg 5000.0 :viscosity-cp 9500.0
                :voc-content-g-per-l 35.0 :fineness-of-grind-hegman 6.5
                :verified? true :registered? true
                :shipped-weight-kg 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :exterior-paint :output-form :liquid
                :material "Acrylic Exterior Masonry Paint"
                :weight-kg 8000.0 :viscosity-cp 11000.0
                :voc-content-g-per-l 180.0 :fineness-of-grind-hegman 6.0
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :offset-printing-ink :output-form :paste
                :material "Sheet-Fed Offset Process Black Ink"
                :weight-kg 2000.0 :viscosity-cp 45000.0
                :voc-content-g-per-l 380.0 :fineness-of-grind-hegman 7.0
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"disperser-001" {:id "disperser-001" :kind :high-speed-disperser
                     :verified? true :registered? true
                     :last-maintenance-date "2026-05-01"}
   "mill-002" {:id "mill-002" :kind :bead-mill
               :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `paintmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `paintmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- two verified+registered batches within their own VOC-content
  ceiling (one with shipping headroom, one nearly fully shipped so a
  small new shipment blows through its own logged weight -- HARD
  hold), one UNVERIFIED/unregistered batch (blocks any shipment
  coordinated against it); one verified+registered high-speed-
  disperser unit (schedulable for maintenance), one UNVERIFIED/
  unregistered bead-mill unit (blocks any maintenance scheduling
