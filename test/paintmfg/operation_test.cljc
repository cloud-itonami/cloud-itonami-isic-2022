(ns paintmfg.operation-test
  "Smoke tests for the compiled PaintOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `paintmfg.governor-contract-test`; the Store contract in
  `paintmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "PaintOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "disperser-001" :severity :moderate :description "solvent VOC vapor concentration rising near dispersion equipment"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
